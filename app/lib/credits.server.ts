 ...
 * Implements direct credit deduction with transaction logging
 */
export async function consumeUserCredits(
  ...
  try {
    ...
    if (fetchError && fetchError.code !== "PGRST116") {
      ...
      return {
        success: false,
        error: fetchError.message || "Failed to fetch credits",
      };
    }
    ...
    const availableCredits = (currentCredits.total_credits || 0) - (currentCredits.used_credits || 0);
    ...
    const newBalance = (currentCredits.total_credits || 0) - newUsedCredits;
    ...
    // Log the transaction as 'usage' (valid transaction_type)
...
export async function addUserCredits(
  ...
  try {
    ...
    const oldBalance = currentCredits 
      ? (currentCredits.total_credits || 0) - (currentCredits.used_credits || 0)
      : 0;
    
    const newTotalCredits = (currentCredits?.total_credits || 0) + amount;
    const newBalance = newTotalCredits - (currentCredits?.used_credits || 0);
    ...
    // Create transaction log with correct type based on featureType
    const transactionType = featureType === "bonus" ? "bonus" : "purchase";
